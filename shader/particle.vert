...
uniform float ParcticleSize = 1.0; //Particle size
...
//Offsets to the position in camera coordinates for each vertex of the particles quad
...
cameraPos = (MV* vec4(pos,1)).xyz + (offsets[gl_VertexID] * ParcticleSize);
Transp = mix(1,0,t / ParticleLifetime);
} else {
//Partcle doesn't exist, draw fully transparent
cameraPos = vec3(0);
Transp = 0.0;
}
